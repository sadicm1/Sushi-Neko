...
class GameScene: SKScene {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // grab first sushi piece from suhi tower array
    ...
    func addRandomPieces(total: Int) {
        ...
        for _ in 1...total {
            ...
            if lastPiece?.side != Side.none {
                ...
                let rand = CGFloat.random(0, max: 1.0)
    ...
    func gameOver() {
        ...
        actionToDie(side: character.side)
    }
    ...
    func actionToDie(side: Side) {
        // shows the dramatic falling action when our cat dies
        
        var actionName = ""
        
        // find action name according to character side
        if side == .left {
            actionName = "FallLeft"
        } else if side == .right {
        ...
        // assign and run the death action on character node
        if let deathAction = SKAction(named: actionName) {
            character.run(deathAction)
        }
    }
